 ...
 * @param mask
 */
export function ensureFilePathExists(
  path: string,
  mask: number = parseInt("0777", 8)
): Promise<void> {
 ...
 * @param fileName  filename or file descriptor
 */
export function readFile(fileName: string | Buffer | URL): Promise<Buffer> {
  return new Promise<Buffer>((resolve, reject) => {
    _readFile(fileName, (err: any, data: any) => {
      if (err) reject(err);
...
): Promise<Stats> | PromiseLike<Stats> {
  return new Promise<Stats>((resolve, reject) => {
    _stat(path, (err: any, stats: Stats) => {
...
export function readDir(
  ...
  return new Promise<string[]>((resolve, reject) => {
    _readDir(path, (err: any, files: string[]) => {
      if (err) return reject(err);
